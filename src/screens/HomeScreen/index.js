...
import { getProvider, listOrders } from '../../graphql/queries';
import { updateProvider, updateOrder } from '../../graphql/mutations';
...
const HomeScreen = () => {
  ...
  const fetchProvider = async () => {
    try {
      ...
      const providerData = await API.graphql(
        graphqlOperation(getProvider, { id: userData.attributes.sub }),
      );
  ...
  const onGoPress = async () => {
    ...
    try {
      ...
      const updatedProviderData = await API.graphql(
        graphqlOperation(updateProvider, { input })
      )
      setProvider(updatedProviderData.data.updateProvider);
    } catch (e) {
      console.error(e);
    }
  }
    ...
    if (order) {
      return (
        <View style={{ alignItems: 'center' }}>
          ...
          <Text style={styles.bottomText}>Coming to  {order?.user?.username}</Text>
        </View>
      )
    }
